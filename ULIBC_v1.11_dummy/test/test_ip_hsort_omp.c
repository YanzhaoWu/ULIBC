...
int main(int argc, char **argv) {
  ...
  OMP("omp parallel") {
    ...
    printf("[%8ld,%8ld] => %ld: %d of %d threads\n",
	   (long)ls, (long)le, (long)(le-ls), ni.id, ULIBC_get_num_threads());
    heapsort(&a[ls], le-ls, sizeof(long), compar);
  }
#else
  heapsort(a, len, sizeof(long), compar);
#endif
  t2 = get_msecs();
  printf("(%.2f msec.)\n", t2-t1);
  printf("\n");
